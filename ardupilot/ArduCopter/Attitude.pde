...
get_of_roll(int32_t input_roll)
{
    ...
    int32_t new_roll = 0;
    int32_t p,i,d;
    //change the tot_x_cm to 0 if there is no of update for last 0.5 sec. This also ensures that tot_x_cm changes to 0 while switching between modes.
    if((of_last_update - last_of_roll_update) > 500){
      tot_x_cm = 0;
      p = 0;              
      i = 0;
      d = 0;
    ...
    if( of_last_update != last_of_roll_update){
        last_of_roll_update = of_last_update;
    ...
    // limit max angle
    of_roll = constrain_int32(of_roll, -900, 900);
    //hal.console->printf(("\n of_roll %d ::"),of_roll);
    //hal.console->printf(("input_roll: %d ::"),input_roll);
...
get_of_pitch(int32_t input_pitch){
    ...
    int32_t new_pitch = 0;
    int32_t p,i,d;
    //change the tot_y_cm to 0 if there is no of update for last 0.5 sec. This also ensures that tot_y_cm changes to 0 while switching between modes.
    if((of_last_update - last_of_pitch_update) > 500){
      tot_y_cm = 0;
      p = 0;              
      i = 0;
      d = 0;
    ...
    // check if new optflow data available
    if( of_last_update != last_of_pitch_update ) {
        last_of_pitch_update = of_last_update;
        // add new distance moved
        tot_y_cm += of_y_cm;
        // only stop roll if caller isn't modifying pitch
        if( input_pitch == 0 && current_loc.alt < 1500 ) {
            p = g.pid_optflow_pitch.get_p(tot_y_cm);                      //NOTE: tot_y_cm must be negative but it is positive as the pitch control is by default reversed.
            i = g.pid_optflow_pitch.get_i(tot_y_cm, 0.010f);              // we could use the last update time to calculate the time change
            d = g.pid_optflow_pitch.get_d(tot_y_cm, 0.010f);
            //hal.console->printf((" PP: %d ::"),p);
            //hal.console->printf((" PI: %d ::"),i);
            //hal.console->printf((" PD: %d ::"),d);
            new_pitch = p + i + d;
        }else{
            tot_y_cm = 0;
    ...
    // limit max angle
    of_pitch = constrain_int32(of_pitch, -900, 900);
    //hal.console->printf((" of_pitch %d ::"),of_pitch);
    //hal.console->printf(("input_pitch: %d ::"),input_pitch);
...
get_throttle_rate_stabilized(int16_t target_rate)
{
    ...
    if ((target_rate<0 && !motors.limit.throttle_lower) || (target_rate>0 && !motors.limit.throttle_upper)) {
        controller_desired_alt += target_rate * 0.01f;
    }
